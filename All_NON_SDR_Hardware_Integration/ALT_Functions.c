...
int8_t Init_ALT(const int ALT_fd, struct ALT_CAL * const Altimeter_Calibration_Data)
{
    puts("\nConfigureing Altimeter Control Registers...\n");

    int8_t Reset_Status = DirI2CWrite_CMD_BYTE(ALT_fd, RESET);
    if(Reset_Status < 0)
    {
    ...
    while(1)
    {
        int8_t Reboot_Status = DirI2CRead_LE_Reg_16(ALT_fd, FACTORY_PROM_READ, &Reboot_Reg);
    ...
    if(Reset_Status == 1)
    {
        ...
        int8_t Prom_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, FACTORY_PROM_READ, &(Altimeter_Calibration_Data->FACTORY_DATA));
        ...
        int8_t T1_Sense_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C1_PROM_READ, &(Altimeter_Calibration_Data->T1_SENS));
        ...
        int8_t T1_OFF_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C2_PROM_READ, &(Altimeter_Calibration_Data->T1_OFF));
        ...
        int8_t TCS_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C3_PROM_READ, &(Altimeter_Calibration_Data->TCS));
        ...
        int8_t TCO_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C4_PROM_READ, &(Altimeter_Calibration_Data->TCO));
        ...
        int8_t TREFF_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C5_PROM_READ, &(Altimeter_Calibration_Data->TREFF));
        ...
        int8_t TEMPSENS_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, C6_PROM_READ, &(Altimeter_Calibration_Data->TEMPSENS));
        ...
        int8_t SERIAL_CRC_Read_Status = DirI2CRead_LE_Reg_16(ALT_fd, SERIAL_PROM_READ, &(Altimeter_Calibration_Data->SERIAL_AND_CRC));
...
int32_t Get_Raw_Pressure(const int ALT_fd, const uint16_t Pressure_OSR)
{
    ...
    switch(Pressure_OSR)
    {
        case TWO_e8:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D1_CONV_256);
        ...
        case TWO_e9:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D1_CONV_512);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e10:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D1_CONV_1024);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e11:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D1_CONV_2048);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e12:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D1_CONV_4096);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
    ...
    if(Conversion_Start_Status == 1)
    {
        Bytes_Read = DirI2CRead_Reg_Block8(ALT_fd, ADC_READ, ADC_Pressure_Return, 3);
    ...
    //Return On Fail: -1
    //App Notes: Make sure to pass an Pressure OSR 256, 512, 1024, 2048 or 4096 and no other values
int32_t Get_Raw_Temperature(const int ALT_fd, const int Temperature_OSR)
{
    int8_t Conversion_Start_Status = 0;
    ...
    switch(Temperature_OSR)
    {
        case TWO_e8:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D2_CONV_256);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e9:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D2_CONV_512);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e10:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D2_CONV_1024);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e11:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D2_CONV_2048);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
        ...
        case TWO_e12:
        {
            Conversion_Start_Status = DirI2CWrite_CMD_BYTE(ALT_fd, D2_CONV_4096);
            if(Conversion_Start_Status == 1)
            {
                if(DEBUG == 1)
                {
    ...
    if(Conversion_Start_Status == 1)
    {
        Bytes_Read = DirI2CRead_Reg_Block8(ALT_fd, ADC_READ, ADC_Temp_Return, 3);
...
void Run_Altimeter(const int ALT_fd, struct ALT_CAL * const Altimeter_Calibration_Data, struct ALT_OUT * const Altimeter_Out)
{
    int32_t Raw_Temperature = Get_Raw_Temperature(ALT_fd, 512);
    int32_t Raw_Pressure = Get_Raw_Pressure(ALT_fd, 512);
