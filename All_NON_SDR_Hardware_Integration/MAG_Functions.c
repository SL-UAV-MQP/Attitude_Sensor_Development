...
int8_t Init_MAG(const int MAG_fd)
{
    puts("\nConfigureing Magnotometer Control Registers...\n");

    DirI2CWrite_Reg_8(MAG_fd, CTRL_REG2, INIT_RST_LIS);
    usleep(5000);

    //puts("Configureing Magnotometer Control 1 Register...");
        //See LIS3MDL.h for configuration setting
    int8_t CTRL1_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG1, CTRL_REG1_INIT);
    uint8_t CTRL1_Read = 0;
    int8_t CTRL1_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG1, &CTRL1_Read);
    ...
    //puts("Configureing Magnotometer Control 2 Register...");
        //See LIS3MDL.h for configuration setting
    int8_t CTRL2_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG2, CTRL_REG2_INIT);
    uint8_t CTRL2_Read = 0;
    int8_t CTRL2_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG2, &CTRL2_Read);
    ...
    //puts("Configureing Magnotometer Control 3 Register...");
        //See LIS3MDL.h for configuration setting
    int8_t CTRL3_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG3, CTRL_REG3_INIT);
    uint8_t CTRL3_Read = 0;
    int8_t CTRL3_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG3, &CTRL3_Read);
    ...
    //puts("Configureing Magnotometer Control 4 Register...");
        //See LIS3MDL.h for configuration setting
    int8_t CTRL4_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG4, CTRL_REG4_INIT);
    uint8_t CTRL4_Read = 0;
    int8_t CTRL4_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG4, &CTRL4_Read);
    ...
    //puts("Configureing Magnotometer Control 5 Register...");
        //See LIS3MDL.h for configuration setting
    int8_t CTRL5_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG5, CTRL_REG5_INIT);
    uint8_t CTRL5_Read = 0;
    int8_t CTRL5_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG5, &CTRL5_Read);
...
int8_t Get_Raw_X_Magnetometer(const int MAG_fd, int16_t * const Raw_X_MAG)
{
    uint8_t X_MAG_High = 0;
    int8_t X_MAG_High_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_X_H, &X_MAG_High);

    uint8_t X_MAG_Low = 0;
    int8_t X_MAG_Low_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_X_L, &X_MAG_Low);
...
int8_t Get_Raw_Y_Magnetometer(const int MAG_fd, int16_t * const Raw_Y_MAG)
{
    uint8_t Y_MAG_High = 0;
    int8_t Y_MAG_High_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_Y_H, &Y_MAG_High);
    
    uint8_t Y_MAG_Low = 0;
    int8_t Y_MAG_Low_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_Y_L, &Y_MAG_Low);
...
int8_t Get_Raw_Z_Magnetometer(const int MAG_fd, int16_t * const Raw_Z_MAG)
{
    uint8_t Z_MAG_High = 0;
    int8_t Z_MAG_High_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_Z_H, &Z_MAG_High);
    
    uint8_t Z_MAG_Low = 0;
    int8_t Z_MAG_Low_Read_Status = DirI2CRead_Reg_8(MAG_fd, OUT_Z_L, &Z_MAG_Low);
    ...
    //App Notes: none
int8_t Poll_Magnetometer(const int MAG_fd, const uint8_t MAG_Status_Register, struct MAG_OUT * const MAG_Out)
{
    ...
    uint8_t MAG_FSR_Read_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG2, &MAG_FSR_Check);
    ...
    if(New_X > 0)
    {
        ...
        if(Get_Raw_X_Magnetometer(MAG_fd, &Raw_X_MAG) > 0)
        {
            Get_Success++;
    ...
    if(New_Y > 0)
    {
        ...
        if(Get_Raw_Y_Magnetometer(MAG_fd, &Raw_Y_MAG) > 0)
        {
            Get_Success++;
    ...
    if(New_Z > 0)
    {
        ...
        if(Get_Raw_Z_Magnetometer(MAG_fd, &Raw_Z_MAG) > 0)
        {
            Get_Success++;
...
void FindNorth(struct MAG_OUT * const MAG_Out_ptr, struct DownVector * const DownVectorPtr, struct NorthVector * const NorthVectorPtr)
{
    ...
    //MAG_Out: const pointer to a MAG_OUT structure, Pointer to array where all magnetometer final output data will get stored
    //Return Type: Void
void Run_MAG(const int MAG_fd, struct MAG_OUT * const MAG_Out, struct DownVector * const DownVectorPtr, struct NorthVector * const NorthVectorPtr)
{
    uint8_t Get_Success = 0;
    uint8_t MAG_Data_Status = 0x00;
    uint8_t MAG_Data_SRC_Status = 0x00;

    uint8_t Status_Reg_Poll_Status = DirI2CRead_Reg_8(MAG_fd, STATUS_REG_LIS, &MAG_Data_Status);
    uint8_t SRC_Reg_Poll_Status = DirI2CRead_Reg_8(MAG_fd, STATUS_REG_LIS, &MAG_Data_SRC_Status);
    if(DEBUG == 3)
    {
    ...
    if(Status_Reg_Poll_Status > 0)
    {
        ...
        if(New_Data > 0)
        {
            int8_t Poll_MAG_Status = Poll_Magnetometer(MAG_fd, MAG_Data_Status, MAG_Out);
...
void Run_MAG_Self_Test(const int MAG_fd, struct MAG_OUT * const MAG_Out)
{
    Dash_Line();

    uint8_t Self_Test_Register_Setting = (CTRL_REG1_INIT | SELF_TEST_MASK);
    int8_t Self_Test_Write_Status = DirI2CWrite_Reg_8(MAG_fd, CTRL_REG1, Self_Test_Register_Setting);
    uint8_t Self_Test_Read = 0;
    int8_t Self_Test_Status = DirI2CRead_Reg_8(MAG_fd, CTRL_REG1, &Self_Test_Read);
    if(Self_Test_Read == Self_Test_Register_Setting)
    {
        ...
        while(Sample_Set < MAG_SELF_TEST_DURRATION)
        {
            ...
            if((MAG_Data_Status & MAG_FULL_NEW) > 0)
            {
                Sample_Set++;
                int8_t Poll_MAG_Status = Poll_Magnetometer(MAG_fd, MAG_Data_Status, MAG_Out);
                uint8_t SRC_Reg_Poll_Status = DirI2CRead_Reg_8(MAG_fd, STATUS_REG_LIS, &MAG_Overflow_Status);
