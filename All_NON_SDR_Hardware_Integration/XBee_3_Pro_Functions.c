...
int8_t Enable_CTSRTS(const int XBEE_fd)
{
    struct termios options ;
  
// Get and modify current options:

    if(tcgetattr (XBEE_fd, &options) != 0)
    {
        puts("Enabling CTS/RTS Checks [BAD]");
        return(-1);
    }
    ...
    if(tcsetattr (XBEE_fd, TCSANOW, &options) != 0)
    {
        puts("Enabling CTS/RTS Checks [BAD]");
        return(-1);
    }
    ...
    {
        usleep (10000) ;
        puts("Enabling CTS/RTS Checks [GOOD]");
        return(1);
    }
}

int8_t Disable_CTSRTS(const int XBEE_fd)
{
    struct termios options ;
...
// Get and modify current options:
    ...
    if(tcgetattr (XBEE_fd, &options) != 0)
    {
        puts("Disabling CTS/RTS Checks [BAD]");
        return(-1);
    }
    ...
    {
        usleep (10000) ;
        puts("Disabling CTS/RTS Checks [GOOD]");
        return(1);
    }
}


int16_t Serial_Puts_CTSRTS_Safe(const int fd, const char * Serial_MSG, uint8_t FLAGS)
{
    size_t MSG_Length = strlen(Serial_MSG);
    ssize_t Bytes_In_Buff  = 0;
    int8_t Buffer_Is_Full = 0;

    if(ioctl(fd, TIOCOUTQ, &Bytes_In_Buff) < 0)
    {
        Buffer_Is_Full = -1;
    }
    else if((4096 - Bytes_In_Buff) < (ssize_t)(MSG_Length))
    {
        
        Buffer_Is_Full = 1;
    }

    if((((FLAGS & O_FORCE) == 0) && (Buffer_Is_Full == 0)) || ((FLAGS & O_FORCE) == 1))
    {
        if((Buffer_Is_Full == 1) && (FLAGS && O_FLUSH == 1))
        {
            tcflush (fd, TCIOFLUSH);
        }

        ssize_t bytes_written = write(fd, Serial_MSG, MSG_Length);
        if ((bytes_written == -1) || (bytes_written != (ssize_t)(MSG_Length)))
        {
            if((bytes_written == EAGAIN) && ((FLAGS & O_FORCE) == 1))
            {
                printf("[%lu / %lu] Bytes Force Writen.\n", MSG_Length, bytes_written);
                return(bytes_written);
            }
            else
            {
                puts("Failed To Write Some/Any Bytes.");
                return(-3);
            }
        }
        else
    ...
    else if(Buffer_Is_Full < 0)
    {
        puts("[WARNING] Could Not Get UART Buffer Capacity.");
        return(-1);
    }
    else if(((FLAGS & O_FORCE) == 0) && (Buffer_Is_Full == 1))
    {
        if(((FLAGS & O_FORCE) == 0))
        printf("UART Buffer Contains [%lu / 4096] Bytes. Could Not Write %lu Bytes Without Force.\n", Bytes_In_Buff, MSG_Length);
        return(-2);
    }
    
    puts("[WARNING] Serial_Puts_CTSRTS_Safe Reached Unknown Edge Case State.");
    return(-4);
}
...
int8_t Init_XBee(int * const XBEE_fd)
{
    ...
    if(UART_Port_Setup_Status > 0)
    {
        ...
        puts("\nConfigureing XBee Communication Registers...\n");
        ...
        {   
            ...
            int8_t Enable_CTSRTS_Status = Enable_CTSRTS(*XBEE_fd);

            puts("XBee Configuration Complete!");
