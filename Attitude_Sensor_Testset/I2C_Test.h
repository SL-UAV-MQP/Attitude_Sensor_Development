...
#include "MS5607_02BA03.h"
    ...
    //Set 0 for main running, 1 for Altimiter debug, 2 for IMU debug, 3 for Mag debug. Any > 0 will put GPIO into debug verbose mode for byte by byte examination.
#define DEBUG 0
...
void INS_COMPUTE(const uint64_t, struct ACC_OUT * const, struct GYRO_OUT * const, struct DownVector * const, struct ROLLING_INERTIAL_SUMS * const);
...
void Run_MAG(const int, struct MAG_OUT * const, struct DownVector * const, struct NorthVector * const);
    ...
    //Top level Non Sensor Spesific defined structures
