...
void Init_Attitude_Sensors(const int ALT, const int IMU, const int MAG, struct GYRO_OUT * const GYRO_Out, struct ACC_OUT * const ACC_Out, struct ALT_CAL * const Altimeter_Calibration_Data, struct GYRO_CAL * const Gyrometer_Calibration_Data, struct ACC_CAL * const Accelerometer_Calibration_Data, struct MAG_OUT * const MAG_Out)
{
    int8_t ALT_Status = Init_ALT(ALT, Altimeter_Calibration_Data);
        sleep(2);

    Dash_Line();

    int8_t IMU_Status = Init_IMU(IMU);
    sleep(2);
    if((DEBUG == 0))
    {
        Zeroize_IMU(IMU, GYRO_Out, ACC_Out, Gyrometer_Calibration_Data, Accelerometer_Calibration_Data);
    }
    if(DEBUG == 2)
    {
        sleep(2);
    ...
    int8_t MAG_Status = Init_MAG(MAG);
    sleep(2);
    Run_MAG_Self_Test(MAG, MAG_Out);
    if(DEBUG == 3)
    {
        sleep(2);
    }
