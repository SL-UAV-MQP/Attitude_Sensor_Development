...
int8_t Init_IMU(const int IMU)
{
    ...
    int8_t Reboot_Status = DirI2CWrite_Reg_8(IMU, CTRL3_C, INIT_REBOOT_LSM);
    uint8_t Reboot_Reg = 0x00;
    while(1)
    {
        DirI2CRead_Reg_8(IMU, CTRL3_C, &Reboot_Reg);
        ...
        usleep(1000);
        Reboot_Status = DirI2CWrite_Reg_8(IMU, CTRL3_C, INIT_REBOOT_LSM);
    }
    ...
    DirI2CWrite_Reg_8(IMU, CTRL3_C, INIT_RST_LSM);
    usleep(25);

    uint8_t Reset_Reg = INIT_RST_LSM;
    while(1)
    {
        DirI2CRead_Reg_8(IMU, CTRL3_C, &Reset_Reg);
    ...
    int8_t FUNC_CFG_ACCESS_Write_Status = DirI2CWrite_Reg_8(IMU, FUNC_CFG_ACCESS, FUNC_CFG_ACCESS_INIT);
    uint8_t FUNC_CFG_ACCESS_Read = 0;
    int8_t FUNC_CFG_ACCESS_Read_Status = DirI2CRead_Reg_8(IMU, FUNC_CFG_ACCESS, &FUNC_CFG_ACCESS_Read);
    ...
    else
    {
        ...
        //See LSM6DSO.h for configuration setting
    int8_t PIN_CTRL_Write_Status = DirI2CWrite_Reg_8(IMU, DSO_PIN_CTRL, PIN_CTRL_INIT);
    uint8_t PIN_CTRL_Read = 0;
    int8_t PIN_CTRL_Read_Status = DirI2CRead_Reg_8(IMU, DSO_PIN_CTRL, &PIN_CTRL_Read);
    ...
    else
    {
        ...
        //See LSM6DSO.h for configuration setting
    int8_t FIFO_CTRL4_Write_Status = DirI2CWrite_Reg_8(IMU, DSO_FIFO_CTRL4, FIFO_CTRL4_INIT);
    uint8_t FIFO_CTRL4_Read = 0;
    int8_t FIFO_CTRL4_Read_Status = DirI2CRead_Reg_8(IMU, DSO_FIFO_CTRL4, &FIFO_CTRL4_Read);
    ...
    else
    {
        ...
        //See LSM6DSO.h for configuration setting
    int8_t CTRL1_XL_Write_Status = DirI2CWrite_Reg_8(IMU, CTRL1_XL, CTRL1_XL_INIT);
    uint8_t CTRL1_XL_Read = 0;
    int8_t CTRL1_XL_Read_Status = DirI2CRead_Reg_8(IMU, CTRL1_XL, &CTRL1_XL_Read);
    ...
    else
    {
        ...
        //See LSM6DSO.h for configuration setting
    int8_t CTRL2_G_Write_Status = DirI2CWrite_Reg_8(IMU, CTRL2_G, CTRL2_G_INIT);
    uint8_t CTRL2_G_Read = 0;
    int8_t CTRL2_G_Read_Status = DirI2CRead_Reg_8(IMU, CTRL2_G, &CTRL2_G_Read);
    ...
    else
    {
        ...
        //See LSM6DSO.h for configuration setting
    int8_t CTRL4_C_Write_Status = DirI2CWrite_Reg_8(IMU, CTRL4_C, CTRL4_C_INIT);
    uint8_t CTRL4_C_Read = 0;
    int8_t CTRL4_C_Read_Status = DirI2CRead_Reg_8(IMU, CTRL4_C, &CTRL4_C_Read);
...
int8_t Get_IMU_Raw_Temperature(const int IMU, int16_t * const Raw_Temp)
{
    uint8_t Temp_High = 0;
    int8_t Temp_High_Read_Status = DirI2CRead_Reg_8(IMU, OUT_TEMP_H, &Temp_High);
    
    uint8_t Temp_Low = 0;
    int8_t Temp_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUT_TEMP_L, &Temp_Low);
...
int8_t Get_Raw_X_Gyroscope(const int IMU, int16_t * const Raw_X_Rot)
{
    uint8_t X_GA_High = 0;
    int8_t X_GA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTX_H_G, &X_GA_High);

    uint8_t X_GA_Low = 0;
    int8_t X_GA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTX_L_G, &X_GA_Low);
...
int8_t Get_Raw_Y_Gyroscope(const int IMU, int16_t * const Raw_Y_Rot)
{
    uint8_t Y_GA_High = 0;
    int8_t Y_GA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTY_H_G, &Y_GA_High);
    
    uint8_t Y_GA_Low = 0;
    int8_t Y_GA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTY_L_G, &Y_GA_Low);
...
int8_t Get_Raw_Z_Gyroscope(const int IMU, int16_t * const Raw_Z_Rot)
{
    uint8_t Z_GA_High = 0;
    int8_t Z_GA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTZ_H_G, &Z_GA_High);
    
    uint8_t Z_GA_Low = 0;
    int8_t Z_GA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTZ_L_G, &Z_GA_Low);
...
int8_t Get_Raw_X_Linear_Acceleration(const int IMU, int16_t * const Raw_X_Lin)
{
    uint8_t X_LA_High = 0;
    int8_t X_LA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTX_H_A, &X_LA_High);
    
    uint8_t X_LA_Low = 0;
    int8_t X_LA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTX_L_A, &X_LA_Low);
...
int8_t Get_Raw_Y_Linear_Acceleration(const int IMU, int16_t * const Raw_Y_Lin)
{
    uint8_t Y_LA_High = 0;
    int8_t Y_LA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTY_H_A, &Y_LA_High);
    
    uint8_t Y_LA_Low = 0;
    int8_t Y_LA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTY_L_A, &Y_LA_Low);
...
int8_t Get_Raw_Z_Linear_Acceleration(const int IMU, int16_t * const Raw_Z_Lin)
{
    uint8_t Z_LA_High = 0;
    int8_t Z_LA_High_Read_Status = DirI2CRead_Reg_8(IMU, OUTZ_H_A, &Z_LA_High);
    
    uint8_t Z_LA_Low = 0;
    int8_t Z_LA_Low_Read_Status = DirI2CRead_Reg_8(IMU, OUTZ_L_A, &Z_LA_Low);
    ...
    //App Notes: None
int8_t Poll_Gyroscope(const int IMU, struct GYRO_OUT * const GYRO_Out, struct GYRO_CAL * const Gyrometer_Calibration_Data)
{
    uint8_t Get_Success = 0;
    uint8_t Rot_FSR_Check = 0x00;
    uint8_t Rot_FSR_Read_Status = DirI2CRead_Reg_8(IMU, CTRL2_G, &Rot_FSR_Check);
    ...
    if(Get_Raw_X_Gyroscope(IMU, &Raw_X_GR) > 0)
    {
        Get_Success++;
    ...
    if(Get_Raw_Y_Gyroscope(IMU, &Raw_Y_GR) > 0)
    {
        Get_Success++;
    ...
    if(Get_Raw_Z_Gyroscope(IMU, &Raw_Z_GR) > 0)
    {
        Get_Success++;
    ...
    //Return On Fail: number of failed register reads (-1 to -3)
    //App Notes: none
int8_t Poll_Accelerometer(const int IMU, struct ACC_OUT * const ACC_Out, struct ACC_CAL * const Accelerometer_Calibration_Data)
{
    uint8_t Get_Success = 0;
    uint8_t Lin_FSR_Check = 0x00;
    uint8_t Lin_FSR_Read_Status = DirI2CRead_Reg_8(IMU, CTRL1_XL, &Lin_FSR_Check);
    ...
    if(Get_Raw_X_Linear_Acceleration(IMU, &Raw_X_Acc) > 0)
    {
        Get_Success++;
    ...
    if(Get_Raw_Y_Linear_Acceleration(IMU, &Raw_Y_Acc) > 0)
    {
        Get_Success++;
    ...
    if(Get_Raw_Z_Linear_Acceleration(IMU, &Raw_Z_Acc) > 0)
    {   
        Get_Success++;
...
void Zeroize_IMU(const int IMU, struct GYRO_OUT * const GYRO_Out, struct ACC_OUT * const ACC_Out, struct GYRO_CAL * const Gyrometer_Calibration_Data, struct ACC_CAL * const Accelerometer_Calibration_Data)
{
    puts("\nZeroizing IMU...");
    float X_Rot_Cal = 0;
    float Y_Rot_Cal = 0;
    ...
    while((imu_gyro_cal < GYRO_CAL_DURRATION) && (imu_accel_cal < ACC_CAL_DURRATION))
    {
        ...
        uint8_t Status_Reg_Poll_Status = DirI2CRead_Reg_8(IMU, STATUS_REG_LSM, &IMU_Data_Status);
        if(Status_Reg_Poll_Status > 0)
        {
            ...
            {
                int8_t Poll_Gyro_Status = Poll_Gyroscope(IMU, GYRO_Out, Gyrometer_Calibration_Data);
            ...
            {
                int8_t Poll_Acc_Status = Poll_Accelerometer(IMU, ACC_Out, Accelerometer_Calibration_Data);
...
void FindDown(struct ACC_OUT * const ACC_OUT_ptr, struct DownVector * const DownVectorPtr)
{
    ...
    //Accelerometer_Calibration_Data: const pointer to a ACC_CAL structure, Pointer to array where all accelerometer calibration values will get stored
    //Return Type: Void
void Run_IMU(const int IMU, struct GYRO_OUT * const GYRO_Out, struct ACC_OUT * const ACC_Out, struct GYRO_CAL * const Gyrometer_Calibration_Data, struct ACC_CAL * const Accelerometer_Calibration_Data, struct DownVector * const DownVectorPtr)
{
    uint8_t IMU_Data_Status = 0x00;
    uint8_t Status_Reg_Poll_Status = DirI2CRead_Reg_8(IMU, STATUS_REG_LSM, &IMU_Data_Status);
    if(Status_Reg_Poll_Status > 0)
    {
        ...
        if(New_Temp > 0)
        {
            ...
            int8_t Get_Temp_Status = Get_IMU_Raw_Temperature(IMU, &IMU_Raw_Temp);
        }
        else
        {
            puts("\n");
...
void INS_COMPUTE(const uint64_t Interval, struct ACC_OUT * const ACC_Out, struct GYRO_OUT * const GYRO_Out, struct DownVector * const DownVectorPtr, struct ROLLING_INERTIAL_SUMS * const INS)
{
    double Delta_T = Interval/1e6;
